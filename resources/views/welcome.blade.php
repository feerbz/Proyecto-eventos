...
<body class="antialiased bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white min-h-screen flex flex-col relative overflow-x-hidden">
    ...
    <nav class="w-full p-6 flex justify-between items-center max-w-7xl mx-auto z-10 relative">
        ...
        <div class="font-black text-2xl tracking-tighter flex items-center gap-2">
            <span class="text-emerald-600">Uni</span>
            <span>Event</span>
            <span class="text-emerald-600">.</span>

            <span class="text-xs font-bold px-2 py-1 bg-gray-200 dark:bg-gray-800 rounded-md text-gray-500 uppercase tracking-widest hidden sm:inline-block">
                UPIICSA · IPN
            </span>
        </div>
        ...
        <div class="flex gap-4 items-center">
            ...
            @auth
                <a href="{{ route('dashboard') }}"
                   class="text-sm font-bold px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition-all shadow-sm">
                    Ir a mi Panel
                </a>
            @else
                <a href="{{ route('login') }}"
                   class="text-sm font-bold text-gray-600 dark:text-gray-400 hover:text-emerald-500 transition-colors">
                    Iniciar Sesión
                </a>
            @endauth
        ...
        <div class="text-center max-w-4xl mx-auto w-full">
            ...
            @guest
                <div class="flex flex-col sm:flex-row justify-center items-center gap-4 mt-8">
                    ...
                    <a href="{{ route('login') }}"
                       class="w-full sm:w-64 px-8 py-4 bg-white dark:bg-gray-800 text-gray-900 dark:text-white font-bold rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all text-center">
                        Iniciar Sesión
                    </a>

                    <a href="{{ route('register') }}"
                       class="w-full sm:w-64 px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl shadow-lg shadow-emerald-500/30 transition-all active:scale-95 flex items-center justify-center gap-2 group">
            ...
            @else
                ...
                <div class="flex justify-center mt-8">
                    ...
                    <a href="{{ route('dashboard') }}"
                       class="px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl shadow-lg shadow-emerald-500/30 transition-all active:scale-95 flex items-center justify-center gap-2 group">
